...
DECLARE
	...
	v_where_new_skus TEXT := ' WHERE UPPER(country) = UPPER((SELECT country FROM "tEvent"  WHERE "eventId" = ' || p_event_id || ')) ';
...
ALTER FUNCTION public.get_products_by_filters(p_event_id integer, p_skus text[], p_part_numbers text[], p_supplier_ids text[], p_not_supplier_ids text[], p_supplier_names text[], p_brands text[], p_not_brands text[], p_ic1 text[], p_not_ic1 text[], p_ic2 text[], p_not_ic2 text[], p_ic3 text[], p_not_ic3 text[], p_ic4 text[], p_not_ic4 text[], p_part_descriptions text[], p_selected boolean, p_new boolean, p_offerid integer, p_offerno integer, p_offertypeid integer, p_searchedat timestamp without time zone, p_sort_sku_desc boolean, p_sort_partno_desc boolean, p_sort_desc_desc boolean, p_sort_brand_desc boolean, p_sort_ic4_desc boolean, p_sort_showroom_desc boolean, p_sort_clearance_desc boolean, p_page_number integer, p_page_size integer)

-- View: public."vwUpdHybExpLoyFixedPctDiscNZ"

-- DROP VIEW public."vwUpdHybExpLoyFixedPctDiscNZ";

CREATE OR REPLACE VIEW public."vwUpdHybExpLoyFixedPctDiscNZ" AS
 WITH base AS (
         SELECT eo."eventId" AS event_id,
            ...
            'Catalogue Rewards Offer'::character varying AS "PILL_TEXT",
        ...
        UNION ALL
         SELECT eo."eventId" AS event_id,
           ...
           FROM "tEvent" ev
             ...
             JOIN "tOfferType" ot ON eo."commercialOfferType"::text = ot."offerType"::text AND ev.country::text = ot.country::text
             LEFT JOIN "tHybrisStickerText" hst ON eo."hybrisStickerText"::text = hst."hybrisStickerText"::text AND ev.country::text = hst.country::text
          WHERE ev.country::text = 'NZ'::text AND ev.locked = true AND eo."isNotAvailableOnline" = false AND eod."advertisedPrice" > 0::numeric AND (ot."offerTypeId" = ANY (ARRAY[6, 106])) AND COALESCE(eod."fromPriceIndicator", false) = false AND eo."isRewards" = true AND NOT (ev."eventType"::text = 'Retail Catalogue'::text AND eo."pagePosition" = 0)
        ...
        UNION ALL
         SELECT eo."eventId" AS event_id,
           ...
           FROM "tEvent" ev
             ...
             JOIN "tConfig" tc ON ev.country::text = tc.configkey::text AND tc.configtype::text = 'COUNTRY'::text
             LEFT JOIN "tHybrisStickerText" hst ON eo."hybrisStickerText"::text = hst."hybrisStickerText"::text AND ev.country::text = hst.country::text
          WHERE ev.country::text = 'NZ'::text AND ev.locked = true AND eo."isNotAvailableOnline" = false AND eod."advertisedPrice" > 0::numeric AND (ot."offerTypeId" = ANY (ARRAY[13, 23])) AND eo."isRewards" = true AND NOT (ev."eventType"::text = 'Retail Catalogue'::text AND eo."pagePosition" = 0)
        ...
        UNION ALL
         SELECT eo."eventId" AS event_id,
           ...
           FROM "tEvent" ev
             ...
             JOIN "tConfig" tc ON ev.country::text = tc.configkey::text AND tc.configtype::text = 'COUNTRY'::text
             LEFT JOIN "tHybrisStickerText" hst ON eo."hybrisStickerText"::text = hst."hybrisStickerText"::text AND ev.country::text = hst.country::text
          WHERE ev.country::text = 'NZ'::text AND ev.locked = true AND eo."isNotAvailableOnline" = false AND eod."advertisedPrice" > 0::numeric AND ot."offerTypeId" = 14 AND eo."isRewards" = true AND NOT (ev."eventType"::text = 'Retail Catalogue'::text AND eo."pagePosition" = 0)
...
ALTER TABLE public."vwUpdHybExpLoyFixedPctDiscNZ"

-- View: public."vwUpdHybExpLoyComboDtlAU"

-- DROP VIEW public."vwUpdHybExpLoyComboDtlAU";

CREATE OR REPLACE VIEW public."vwUpdHybExpLoyComboDtlAU" AS
 WITH base AS (
         SELECT ev."eventId",
            eo.page,
            eo."pagePosition",
            ...
            eo."offerNumber",
            ot."offerTypeId",
            concat('C', 'AUS', 'E', ev."eventId"::text, 'P', eo.page::text, 'P',
                CASE
          ...
          WHERE ev.locked = true AND eo."isNotAvailableOnline" = false AND eo."advertisedPrice" > 0::numeric AND ev.country::text = 'AU'::text AND eo."isRewards" = true AND (ot."offerTypeId" = ANY (ARRAY[3, 103, 15, 115, 25]))
...
ALTER TABLE public."vwUpdHybExpLoyComboDtlAU"
